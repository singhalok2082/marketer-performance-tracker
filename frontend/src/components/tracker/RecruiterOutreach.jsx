...
export default function RecruiterOutreach({ user }) {
  ...
  const load = useCallback(() => {
    ...
    api.get("/outreach", { params })
      ...
      .catch(() => setMsg({ type: "error", text: "Failed to load inbound requirements" }))
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      setMsg({ type: "success", text: editingId ? "Requirement updated." : "Requirement logged." });
      setShowForm(false);
      load();
    } catch (err) {
  ...
  const remove = async (r) => {
    if (!confirm(`Remove this requirement (${r.vendor_company || "unnamed vendor"})?`)) return;
    try {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="text-sm text-muted">{rows.length} requirement{rows.length === 1 ? "" : "s"}</div>
      ...
      <div className="card overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-10 text-muted">No inbound requirements logged yet.</div>
        ) : (
