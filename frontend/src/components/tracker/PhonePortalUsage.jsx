...
const emptyForm = { phone_number: "", resume_id: "", portal_ids: [], custom_portal_names: [] };

export default function PhonePortalUsage({ user }) {
  ...
  const load = useCallback(() => {
    ...
    api.get("/phone-numbers/portal-usage", { params })
      ...
      .catch(() => setMsg({ type: "error", text: "Failed to load phone numbers" }))
  ...
  const submit = async (ev) => {
    ...
    try {
      ...
      if (editingId) await api.patch(`/phone-numbers/portal-usage/${editingId}`, payload);
      else await api.post("/phone-numbers/portal-usage", payload);
      setMsg({ type: "success", text: editingId ? "Phone number updated." : "Phone number added." });
      setShowForm(false);
      load();
    } catch (err) {
  ...
  const remove = async (e) => {
    if (!confirm(`Remove ${e.phone_number}?`)) return;
    try {
      await api.delete(`/phone-numbers/portal-usage/${e.id}`);
      load();
    } catch {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h3 className="text-sm font-bold mb-1">Use of the phone number on different portals</h3>
        <p className="text-sm text-muted -mb-1">
          Track which resume and portals go with each phone number you use to create job-portal profiles.
        </p>
      </div>
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="text-sm text-muted">{entries.length} phone number{entries.length === 1 ? "" : "s"}</div>
        ...
        <form onSubmit={submit} className="card p-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="sm:col-span-2 flex justify-end">
            <button type="submit" disabled={saving} className="btn-primary disabled:opacity-40">
              {saving ? "Saving…" : editingId ? "Save changes" : "Add phone number"}
            </button>
          </div>
        </form>
      )}
      ...
      <div className="card overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-10 text-muted">No phone numbers logged yet.</div>
        ) : (
              ...
              <tbody>
                {entries.map(e => (
                  <tr key={e.id} className="border-t border-border hover:bg-surface transition-colors">
                    ...
                    <td className="px-4 py-2.5 text-muted">{e.resume_tech_stack || "—"}</td>
